...
function UnionSchema(schemas, names) {
  ...
  for (i = 0; i < length; i++) {
    ...
    if (typeof curSchema === 'string' && names.hasName(curSchema, null)) {
      newSchema = names.getName(curSchema, null);
    } else {
      try {
        newSchema = makeAvscObject(curSchema, names);
      } catch (err) {
        throw new Error('Union item must be a valid Avro schema: ' + err);
      }
    }

    // Check the new schema.
    tmpType = newSchema.type;
    if (VALID_TYPES.indexOf(tmpType) !== -1
        && NAMED_TYPES.indexOf(tmpType) === -1
        && allTypes.indexOf(tmpType)) {
      throw new Error(tmpType + ' type already in Union');
    }
...
UnionSchema.prototype.toJSON = function (names) {
  if (!names) {
    names = new Names();
  }
  ...
  this.schemas.forEach(function (ele) {
    toDump.push(ele.toJSON(names));
  });

  return toDump;
};

function ErrorUnionSchema(schemas, names) {
  UnionSchema.call(this, schemas.push('string'), names);
}

util.inherits(ErrorUnionSchema, UnionSchema);

ErrorUnionSchema.prototype.toJSON = function (names) {
  if (!names) {
    names = new Names();
  }
